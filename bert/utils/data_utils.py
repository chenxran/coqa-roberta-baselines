...
def sanitize_input(sample_batch, config, tokenizer, training=True):
    """
    ...
    for ex in sample_batch:
        ...
        # print(len(question), len(evidence))
        temp_q.append(question)
        temp_e.append(evidence)
        ...
        # sanitized_batch['question'].append(tokenizer.convert_tokens_to_ids(question))
        # sanitized_batch['evidence'].append(tokenizer.convert_tokens_to_ids(evidence))
        ...
        sanitized_batch['length'].append((len(question), len(evidence)))
        ex['targets'][0] += 2 + len(question)  # TODO: Roberta/Bert need changes here
        ex['targets'][1] += 2 + len(question)  # TODO: Roberta/Bert need changes here
    ...
    sanitized_batch['encoding'] = tokenizer(
        temp_q, 
        temp_e,
...
def vectorize_input(batch, config, training=True, device=None):
    ...
    # Relevant parameters:
    batch_size = len(batch['encoding']['input_ids'])
    ...
    input_ids = batch['encoding']['input_ids'].to(device)
    attention_mask = batch['encoding']['attention_mask'].to(device)
    token_type_ids = batch['encoding']['token_type_ids'].to(device)
