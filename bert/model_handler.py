...
class ModelHandler(object):
    ...
    def _run_epoch(self, data_loader, training=True, verbose=10, out_predictions=False):
        ...
        with tqdm(total=len(data_loader), ncols=160) as pbar:
            for step, input_batch in enumerate(data_loader):
                input_batch = sanitize_input(input_batch, self.config, tokenizer=self.tokenizer, training=training)
                ...
                # print("model.predict 前", meminfo.used)
                res = self.model.predict(step, x_batch, update=training, out_predictions=out_predictions)
                # print("model.predict 后", meminfo.used)
                ...
                if (verbose > 0) and (step % verbose == 0):
                    ...
                    report = self.report(step, loss, f1 * 100, em * 100, mode)
    ...
    #     if mode == "train":
    #         format_str = "[train: EPOCH {}] step: [{} / {}] | exs = {} | loss = {:0.4f} | f1 = {:0.2f} | em = {:0.2f}"
    #         return format_str.format(self._epoch, step, self._n_train_batches, self._n_train_examples, loss, f1, em)
    #     elif mode == "dev":
    #         return "[predict: EPOCH {}] step: [{} / {}] | f1 = {:0.2f} | em = {:0.2f}".format(
    ...
    def report(self, step, loss, f1, em, mode='train'):
        if mode == "train":
            format_str = "[train: EPOCH {}] exs = {} | loss = {:0.4f} | f1 = {:0.2f} | em = {:0.2f}"
            return format_str.format(self._epoch, self._n_train_examples, loss, f1, em)
        elif mode == "dev":
            return "[predict: EPOCH {}] f1 = {:0.2f} | em = {:0.2f}".format(
                    self._epoch, f1, em)
        elif mode == "test":
            return "[test] | test_exs = {} | f1 = {:0.2f} | em = {:0.2f}".format(
                    self._n_test_examples, f1, em)
        else:
            raise ValueError('mode = {} not supported.' % mode)
